...
useEffect(() => {
    ...
    const [filteredSportsItems, setFilteredSportsItems] = useState(sportsItems);
    ...
    useEffect(() => {
        if (currentLocation === "All") {
          setFilteredSportsItems(sportsItems);
        } else {
            const filteredItems = sportsItems.filter(item => item.location === currentLocation);
            setFilteredSportsItems(filteredItems);
        }
    }, [currentLocation, sportsItems]);
      ...
      <Navbar />
        <div className='flex justify-center bg-black p-3'>
            <div className='max-w-7xl flex flex-row gap-[5%] justify-center '>
                ...
                <div className='w-70%'>
                    {filteredSportsItems.map((curr, index) => (
                        <Card key={index} gymName={curr.name} location={curr.location} type="sports" />
                    ))}
                </div>
            </div>
        </div>
